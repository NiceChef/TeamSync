...
def ensure_sqlite_schema(engine) -> None:
    ...
    if 'role' not in cols:
        _sqlite_add_column(engine, 'users', "role VARCHAR(20) NOT NULL DEFAULT 'client'")
    if 'approval_status' not in cols:
        _sqlite_add_column(engine, 'users', "approval_status VARCHAR(20) NOT NULL DEFAULT 'pending'")
    ...
    if 'tasks' in tables:
        ...
        if 'organization_id' not in tcols:
    ...
    if 'projects' in tables:
        pcols = _sqlite_columns(engine, 'projects')
        if 'planned_start' not in pcols:
            _sqlite_add_column(engine, 'projects', 'planned_start DATETIME')
        if 'deadline' not in pcols:
