...
// Zadania z datą planu lub deadline do nakładki na kalendarzu.
export async function fetchTasksWithDeadline() {
    ...
    return (Array.isArray(data) ? data : []).filter((task) => (
        task.planned_date || task.deadline
    ));
}
...
export function parseTaskDate(value) {
    ...
    const date = value.includes('T') ? parseUTC(value) : new Date(`${value}T00:00:00`);
    return Number.isNaN(date.getTime()) ? null : date;
}

export function getTaskStartDate(task) {
    return parseTaskDate(task.planned_date || task.deadline);
}

export function getTaskEndDate(task) {
    return parseTaskDate(task.deadline || task.planned_date);
}

export function taskTouchesDay(task, day) {
...
export function formatTaskDateRange(task) {
    const start = getTaskStartDate(task);
    const end = getTaskEndDate(task);
    ...
    if (start) return `Od ${formatDateOnly(start)}`;
    if (end) return `Do ${formatDateOnly(end)}`;
