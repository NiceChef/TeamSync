...
function TasksGrid({ isAuthenticated }) {
  ...
  const [sortBy, setSortBy] = useState('soonest_action');
  ...
  const [draftSortBy, setDraftSortBy] = useState('soonest_action');
  ...
  const [draftStatusCodeFilter, setDraftStatusCodeFilter] = useState('');
  const [draftSelectedCategoryFilters, setDraftSelectedCategoryFilters] = useState([]);
  const [draftNoCategories, setDraftNoCategories] = useState(false);
  ...
  const settingsLoaded = useUserSettings({
    ...
    applySettings: (s) => {
      ...
      if (s.sortBy) {
        setSortBy(s.sortBy);
        setDraftSortBy(s.sortBy);
      }

      if (s.sortOrder) {
        setSortOrder(s.sortOrder);
        setDraftSortOrder(s.sortOrder);
      }

      if (s.visibleColumns && setVisibleColumns) {
      ...
      if (s.selectedCategoryFilters) {
        setDraftSelectedCategoryFilters(s.selectedCategoryFilters);
      }

      if (s.noCategories !== undefined) {
        setDraftNoCategories(s.noCategories);
      }
    },
  });
  ...
  const applyTaskFilters = () => {
    ...
    setStatusCodeFilter(draftStatusCodeFilter);

    tasksContext?.setSelectedCategoryFilters?.(draftSelectedCategoryFilters);
    tasksContext?.setNoCategories?.(draftNoCategories);
  };

  const clearDraftTaskFilters = () => {
    setDraftDateFrom('');
    setDraftDateTo('');
    setDraftSortBy('soonest_action');
    setDraftSortOrder('asc');
    setDraftProjectFilter('');
    setDraftPriorityFilter('');
    setDraftStatusCodeFilter('');
    setDraftSelectedCategoryFilters([]);
    setDraftNoCategories(false);
  };

  const clearAndApplyTaskFilters = () => {
    clearDraftTaskFilters();
    ...
    tasksContext?.setSelectedCategoryFilters?.([]);
    tasksContext?.setNoCategories?.(false);
  };
  ...
  const fetchTasks = async () => {
    try {
      ...
      if (statusCodeFilter) {
        url += `&status_code=${statusCodeFilter}`;
      }
  ...
  return (
    <div className="w-full rounded-xl border border-slate-200 bg-white/95 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/95 sm:p-6">
      ...
      <TasksToolbar
        ...
        setProjectFilter={setDraftProjectFilter}
        taskStatuses={taskStatuses}
        statusCodeFilter={draftStatusCodeFilter}
        setStatusCodeFilter={setDraftStatusCodeFilter}
        priorityFilter={draftPriorityFilter}
        setPriorityFilter={setDraftPriorityFilter}
        categories={categories}
        selectedCategoryFilters={draftSelectedCategoryFilters}
        setSelectedCategoryFilters={setDraftSelectedCategoryFilters}
        noCategories={draftNoCategories}
        setNoCategories={setDraftNoCategories}
        visibleColumns={visibleColumns}
        setVisibleColumns={setVisibleColumns}
        onApplyFilters={applyTaskFilters}
        onClearFilters={clearAndApplyTaskFilters}
      />
      ...
      <ActiveTaskFilters
        ...
        setSelectedCategoryFilters={tasksContext?.setSelectedCategoryFilters}
        noCategories={noCategories}
        setNoCategories={tasksContext?.setNoCategories}
        categories={categories}
        dateFrom={dateFrom}
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="inline-flex gap-1 rounded-lg border border-slate-200 bg-white p-1 dark:border-slate-800 dark:bg-slate-900">
          <button
            ...
            className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm font-medium transition ${viewMode === 'table'
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm font-medium transition ${viewMode === 'kanban'
              ? 'bg-indigo-600 text-white'
              : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'
              }`}
          >
