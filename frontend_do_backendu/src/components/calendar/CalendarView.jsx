...
function MonthView({
    ...
    const tasksByDay = useMemo(() => {
        ...
        for (const t of tasks || []) {
            ...
            const key = parseUTC(t.deadline).toDateString();
            const arr = map.get(key) ?? [];
            arr.push(t);
            map.set(key, arr);
        }
...
export default function CalendarView({ isAuthenticated }) {
    ...
    const openCreate = useCallback((date) => {
        ...
        if (startD.getHours() === 0 && startD.getMinutes() === 0) startD.setHours(9);
    ...
    return (
        <div className="space-y-4" onClick={() => selectedEvent && setSelectedEvent(null)}>
